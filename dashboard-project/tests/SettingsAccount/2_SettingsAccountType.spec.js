...
async function companyType(page) {
  ...
  await expect(
    page.getByRole("button", { name: "Company Type" })
  ).toBeVisible();
  ...
  const industryType = page.getByRole("combobox", {
  ...
  await industryType.fill("Test Industry 9014");
  await dropdownSelectOnly(page, industryType);

  const typeName = page.getByRole("textbox", {
    name: "Enter Type Name",
    exact: true,
  });
  const typeNameValue = "Test Company Type 9014";
