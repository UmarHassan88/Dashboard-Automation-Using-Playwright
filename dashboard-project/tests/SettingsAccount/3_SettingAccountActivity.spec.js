...
import { Login, tillSettings, dropdownSelectOnly } from "../utils/helpers.js";
import { type } from "os";

async function accountActivity(page) {
  await Login(page);
  await tillSettings(page);
  const accountOption = page.getByRole("button", {
    name: "Account",
    exact: true,
  });
  await expect(accountOption).toHaveText("Account");
  await accountOption.click();
  const companyActivityButton = page.getByRole("button", {
    name: "Company Activity",
  });
  await expect(companyActivityButton).toBeVisible();
  await companyActivityButton.click();
  const createActivityButton = page.getByRole("button", {
  ...
  const industrySelect = page.getByRole("combobox", {
    name: "Please Select Select industry",
  });
  await industrySelect.click();
  const industryValue = "Test Industry 9014";
  await industrySelect.fill(industryValue);
  await dropdownSelectOnly(page, industrySelect);
  const typeSelect = page.getByRole("combobox", {
    name: "Please Select Select type",
  });
  await typeSelect.click();
  const typeValue = "Test Company Type 9014";
  await page.waitForTimeout(1500);
  await typeSelect.fill(typeValue);
  await dropdownSelectOnly(page, typeSelect);
  const activityName = page.getByRole("textbox", {
    name: "Enter Activity Name",
    exact: true,
  });
  const activityNameValue = "Test Account Activity 9014";
